...
function CandlestickChart({ candles, prediction }: { candles: Candle[]; prediction: Prediction }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-full" style={{ fontFamily: "monospace" }}>
      {ticks.map((t, i) => (
        <line key={i} x1={PAD.left} y1={scaleY(t)} x2={W - PAD.right} y2={scaleY(t)} stroke="#ffffff08" strokeWidth="1" />
      ))}
      ...
      {candles.map((c, i) => {
        ...
        return (
          <g key={c.time}>
            <line x1={x} y1={scaleY(c.high)} x2={x} y2={scaleY(c.low)} stroke={color} strokeWidth="1.5" opacity={isLast ? 1 : 0.75} />
            <rect x={x - candleW / 2} y={bodyTop} width={candleW} height={bodyH} fill={color} opacity={isLast ? 1 : 0.8} rx={1} />
            {isLast && (
              <rect x={x - candleW / 2} y={bodyTop} width={candleW} height={bodyH} fill={color} opacity={0.3} rx={1} filter="url(#glow)" />
            )}
          </g>
        );
      })}

      <line x1={PAD.left} y1={lastY} x2={W - PAD.right} y2={lastY} stroke={lastCandle.close >= lastCandle.open ? "#00e5b0" : "#f04c68"} strokeWidth="1" strokeDasharray="4 3" opacity={0.7} />

      <rect x={W - PAD.right} y={lastY - 9} width={PAD.right - 2} height={18} rx={3} fill={lastCandle.close >= lastCandle.open ? "#00e5b0" : "#f04c68"} />
      <text x={W - PAD.right / 2 - 1} y={lastY + 4} textAnchor="middle" fontSize="9" fontWeight="700" fill="#0a0f1e">
        {lastCandle.close.toFixed(2)}
      </text>

      {ticks.filter((_, i) => i % 2 === 0).map((t, i) => (
        <text key={i} x={PAD.left - 4} y={scaleY(t) + 4} textAnchor="end" fontSize="9" fill="#6b7a99">
          {t.toFixed(1)}
        </text>
      ))}
      ...
      {candles.map((c, i) => {
        ...
        return (
          <div
            ...
            style={{ height: `${pct}%`, background: isUp ? "rgba(0,229,176,0.35)" : "rgba(240,76,104,0.35)" }}
          />
        );
      })}
    </div>
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-6 max-w-3xl">
          ...
          <div className="px-2 pt-2 relative" style={{ height: 230 }}>
            {loading ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                <p className="text-muted-foreground text-sm">Failed to fetch price.</p>
        ...
        <p className="text-center text-xs text-muted-foreground mt-5 leading-relaxed">
          Simulation only — virtual USDC, no real funds at risk. Price data from{" "}
